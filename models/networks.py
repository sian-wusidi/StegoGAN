...
class SoftClamp(nn.Module):
    """
        https://github.com/monniert/dti-sprites/blob/0077829cc45b37cabc75915d7314d38d67ac5562/src/model/tools.py#L145
    """
    def __init__(self, alpha=0.001, inplace=False):
        super().__init__()
        self.alpha = alpha
...
class ResnetMaskV1Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', resnet_layer=1, fusionblock=False):
        ...
        print('***********************************************')
        print('V1->Res-net layer:{}'.format(self.resnet_layer))
        print('***********************************************')

        assert(n_blocks >= 0)
        self.n_blocks = n_blocks
        super(ResnetMaskV1Generator, self).__init__()
    ...
    def forward(self, input, extrafeature=None):
        # Case:
        # Layer 1: The last layer before the resnet blocks, resnet_layer input is not working in this case
        # Layer 2: The resnet blocks
        # In this case, the number resent layer that is used for inserting the extrafeature
...
class ResnetMaskV3Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', input_dim=256, out_dim=256, resnet_layer=1):
        self.resnet_layer = resnet_layer

        print('***********************************************')
        print('V3->Res-net layer:{}'.format(self.resnet_layer))
        print('***********************************************')

        assert(n_blocks >= 0)
        self.n_blocks = n_blocks
        super(ResnetMaskV3Generator, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
...
class NetMatchability(nn.Module):
    def __init__(self, input_dim, out_dim=256, init='kaiming'):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                if init == 'kaiming':
                    ...
                    raise NotImplementedError('Initialization [%s] is not recognized' % init)
